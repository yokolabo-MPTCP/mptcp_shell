...
#Setting
cgn_ctrl=(olia) # lia olia balia wvegas cubic reno
rtt1=(5)
rtt2=(5)   
loss=(0)
queue=(100 500 1000 20000) #0 1 10 50 100 500 1000 2000 20000
duration=100
sleep=0.5 #time[s]
repeat=1
app=3
no_cwr=0
no_rcv=0
qdisc=red # red pfifo_fast
memo="change the parametar(min=1002, max=1003, burst=2) of RED"
		...
		do
			...
			do
				...
				do
					...
					#halfsize=$((limitsize / 10))
					#burst=$((halfsize / 1000 + 1)) 	

					#tc qdisc replace dev eth0 root red limit $limitsize min $halfsize max $limitsize avpkt 1000 burst $burst adaptive probability 0.02 bandwidth 100Mbps ecn harddrop
					#tc qdisc replace dev eth1 root red limit $limitsize min $halfsize max $limitsize avpkt 1000 burst $burst adaptive probability 0.02 bandwidth 100Mbps ecn harddrop

					limit=$((130 * queue[$k]))
					tc qdisc replace dev eth1 root red limit $limit min 1002 max 1003 avpkt 1000 burst 2 adaptive probability 0.02 bandwidth 100Mbps ecn harddrop
					tc qdisc replace dev eth1 root red limit $limit min 1002 max 1003 avpkt 1000 burst 2 adaptive probability 0.02 bandwidth 100Mbps ecn harddrop
				...
				../thput_tex.sh ${today} ${nowdir} ${cgn_ctrl[$z]} ${rtt1[$j]} ${rtt2[$m]} ${loss[$l]} ${repeat}
				k=0
				l=`expr $l + 1`
