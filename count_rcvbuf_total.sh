	...
	do
		...
		do
			...
			do
					...
					echo "${cgn_ctrl[$z]}_rtt1=${rtt1[$j]}_rtt2=${rtt2[$m]}_loss=${loss[$l]}"  >> rcvbuf_for_copy.txt
					while [ $k -lt ${#queue[@]} ]
					do
							...
							do	
								...
								do	
									...
									do				
										...
										buf=$(cat ${nowdir}/${i}th/log/cwnd${p}_subflow${o}_rcv_buf.dat)
										echo "${i}th_APP${p}_subflow${o}_rcv_buf= ${buf}"
										echo "${i}th_APP${p}_subflow${o}_rcv_buf= ${buf}" >> rcvbuf.txt
										total=$((total + buf))				
							...
							ave=$((total / repeat))
							echo "${nowdir}_APP${p} rcvbuf=${total} ave=${ave}"
							echo "${nowdir}_APP${p} rcvbuf=${total} ave=${ave}"  >> rcvbuf.txt

							echo "${ave}"  >> rcvbuf_for_copy.txt
